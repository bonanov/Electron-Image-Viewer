export const TOGGLE_ZOOM_MODE = 'TOGGLE_ZOOM_MODE';
export const UPDATE_BG_COLOR = 'UPDATE_BG_COLOR';
export const ZOOM_FIT = 'ZOOM_FIT';
export const ZOOM_EXPAND = 'ZOOM_EXPAND';
export const ZOOM_FREE = 'ZOOM_FREE';
export const SET_ZOOM_MODE = 'SET_ZOOM_MODE';
export const TOGGLE_SHUFFLE = 'TOGGLE_SHUFFLE';
export const TOGGLE_UI_HIDDEN = 'TOGGLE_UI_HIDDEN';
export const TOGGLE_CROPMODE = 'TOGGLE_CROPMODE';
export const HIDE_UI = 'HIDE_UI';
export const SHOW_UI = 'SHOW_UI';
export const UPDATE_SCALE = 'SET_SCALE';
export const UPDATE_FILESYSTEM = 'UPDATE_FILESYSTEM';
export const SET_SHUFFLE = 'SET_SHUFFLE';
export const UPDATE_IMAGE_POSITION = 'UPDATE_IMAGE_POSITION';
export const RESET_IMAGE_POSITION = 'RESET_IMAGE_POSITION';
export const UPDATE_CURRENT_BLOB = 'UPDATE_CURRENT_BLOB';
export const UPDATE_CONFIG = 'UPDATE_CONFIG';
export const UPDATE_CURRENT_LINK = 'UPDATE_CURRENT_LINK';
export const TOGGLE_SLIDESHOW = 'TOGGLE_SLIDESHOW';
export const DISABLE_SLIDESHOW = 'DISABLE_SLIDESHOW';
export const ENABLE_SLIDESHOW = 'ENABLE_SLIDESHOW';
export const SET_SLIDESHOW_TIMEOUT = 'SET_SLIDESHOW_TIMEOUT';
export const RESET_VIEWMODES = 'RESET_VIEWMODES';

// files
export const UPDATE_FILELIST = 'UPDATE_FILELIST';
export const UPDATE_CURRENT_FILE = 'UPDATE_CURRENT_FILE';
export const UPDATE_FILE_PROPS = 'UPDATE_FILE_PROPS';
export const UPDATE_CURRENT_POSITION = 'UPDATE_CURRENT_POSITION';
export const UPDATE_DIR = 'UPDATE_DIR';
export const UPDATE_BASE64 = 'UPDATE_BASE64';
export const UPDATE_TRASH = 'UPDATE_TRASH';
export const ADD_TO_TRASH = 'ADD_TO_TRASH';
export const REMOVE_FROM_TRASH = 'REMOVE_FROM_TRASH';
export const REMOVE_LAST_FROM_TRASH = 'REMOVE_LAST_FROM_TRASH';
export const REMOVE_FIRST_FROM_TRASH = 'REMOVE_FIRST_FROM_TRASH';
export const CLEAR_TRASH = 'CLEAR_TRASH';
export const SET_TRASH_SEEN = 'SET_TRASH_SEEN';
export const RESET_FILESYSTEM = 'RESET_FILESYSTEM';

// popups
export const TOGGLE_POPUP = 'TOGGLE_POPUP';
export const SET_CONTEXT_MENU_AT = 'SET_CONTEXT_MENU_AT';
export const ADD_POPUP = 'ADD_POPUP';
export const REMOVE_POPUP = 'REMOVE_POPUP';
